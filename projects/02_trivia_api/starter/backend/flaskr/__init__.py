...
def create_app(test_config=None):
    ...
    def retrieve_questions():
        ...
        for question in current_questions:
            current_category.append(question.get("category"))
        current_category = list(dict.fromkeys(current_category))
    ...
    def play_quiz():
        ...
        try:
            ...
            # load all unused questions (not in previous_questions)
            # if "ALL" is selected
            if (selected_category['id'] == 0):
                unused_questions = Question.query.filter(
                                  Question.id.notin_((previous_questions
                                                      ))).all()
            # load unused questions (not in previous_questions)
            # for selected category
            else:
                unused_questions = Question.query.filter_by(
                                category=selected_category['id']).filter(
                                Question.id.notin_((previous_questions
                                                    ))).all()

            new_question = unused_questions[
                           random.randrange(0, len(unused_questions)
                                            )].format() if len(
                                              unused_questions) > 0 else None
